...
module.exports = function (agenda, options) {
  ...
  const getObjectId = function (collection, jobId) {
    if (typeof collection.s.pkFactory !== "function") {
      return ObjectId(jobId);
    }
    return getObjectId(collection, jobId);
  };
  ...
  const getJobs = function (job, state, options) {
    ...
    if (options.query && options.property) {
      if (options.isObjectId) {
        preMatch[options.property] = ObjectId.createFromHexString(
          options.query
        );
  ...
  const requeueJobs = async (jobIds) => {
    ...
    const jobs = await collection
      .find({
        _id: { $in: jobIds.map((jobId) => getObjectId(collection, jobId)) },
      })
      .toArray();
    if (jobs.length === 0) {
